...
2.  **Architecture Adherence:** Strictly adhere to **12-factor application** principles and microservice best practices when building new features or modifying existing ones (e.g., stateless execution, config via environment).
3.  **Performance:** Write efficient, scalable code and optimize Dropwizard configurations.
4.  **Database Integration:** Design and write database models, migrations (e.g., Flyway), and queries for optimal performance using PostgreSQL and JDBI.
...
-   **Microservice Development:** Understanding how to build resilient, decoupled components that fail gracefully and isolate state.
...
3.  Ensure your code adheres to 12-factor principles (e.g., logging to stdout, isolating backing services, strict configuration management).
...
-   **Java / Dropwizard Code:**
    ```java
    // Implementation code here, strictly adhering to
    // 12-factor and microservice principles.
    ```
