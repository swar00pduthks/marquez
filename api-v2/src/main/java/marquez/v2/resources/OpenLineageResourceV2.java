...
public class OpenLineageResourceV2 {
    ...
    public OpenLineageResourceV2(GraphDao graphDao) {
        this.graphDao = graphDao;
        this.graphDao.initGraph(GRAPH_NAME);
    }
    ...
    public Response createLineage(LineageEvent event) {
        ...
        if (event.getJob().getNamespace() != null) {
            ...
            graphDao.upsertNode(GRAPH_NAME, "Source", "name", srcProps);

            Map<String, Object> nsProps = new HashMap<>();
            nsProps.put("name", event.getJob().getNamespace());
            graphDao.upsertNode(GRAPH_NAME, "Namespace", "name", nsProps);
            graphDao.upsertEdge(GRAPH_NAME, "HAS_NAMESPACE", "Source", "name", sourceName, "Namespace", "name", event.getJob().getNamespace());
        }
        ...
        graphDao.upsertNode(GRAPH_NAME, "Job", "fqn", jobProps);
        graphDao.upsertEdge(GRAPH_NAME, "HAS_JOB", "Namespace", "name", event.getJob().getNamespace(), "Job", "fqn", jobFqn);

        // Compute deterministic version hash for JobVersion
        ...
        graphDao.upsertNode(GRAPH_NAME, "JobVersion", "uuid", jvProps);
        graphDao.upsertEdge(GRAPH_NAME, "HAS_VERSION", "Job", "fqn", jobFqn, "JobVersion", "uuid", jobVersionUuid);
        ...
        graphDao.upsertNode(GRAPH_NAME, "Run", "uuid", runProps);
        graphDao.upsertEdge(GRAPH_NAME, "HAS_RUN", "JobVersion", "uuid", jobVersionUuid, "Run", "uuid", runId);
        ...
        graphDao.upsertNode(GRAPH_NAME, "RunState", "uuid", rsProps);
        graphDao.upsertEdge(GRAPH_NAME, "HAS_STATE", "Run", "uuid", runId, "RunState", "uuid", runStateUuid);
        ...
        if (event.getInputs() != null) {
            for (LineageEvent.Dataset ds : event.getInputs()) {
                ...
                graphDao.upsertNode(GRAPH_NAME, "Dataset", "fqn", dsProps);
                graphDao.upsertEdge(GRAPH_NAME, "HAS_DATASET", "Namespace", "name", ds.getNamespace(), "Dataset", "fqn", dsFqn);
                ...
                graphDao.upsertNode(GRAPH_NAME, "DatasetVersion", "uuid", dvProps);
                graphDao.upsertEdge(GRAPH_NAME, "HAS_VERSION", "Dataset", "fqn", dsFqn, "DatasetVersion", "uuid", dvUuid);

                graphDao.upsertEdge(GRAPH_NAME, "CONSUMES_VERSION", "Run", "uuid", runId, "DatasetVersion", "uuid", dvUuid);
            }
        }
        ...
        if (event.getOutputs() != null) {
            for (LineageEvent.Dataset ds : event.getOutputs()) {
                ...
                graphDao.upsertEdge(GRAPH_NAME, "PRODUCES_VERSION", "Run", "uuid", runId, "DatasetVersion", "uuid", dvUuid);
                ...
                if (ds.getFacets() != null && ds.getFacets().getSchema() != null && ds.getFacets().getSchema().getFields() != null) {
                    for (LineageEvent.SchemaField field : ds.getFacets().getSchema().getFields()) {
                        ...
                        graphDao.upsertNode(GRAPH_NAME, "DatasetField", "id", fieldProps);
                        graphDao.upsertEdge(GRAPH_NAME, "HAS_FIELD", "DatasetVersion", "uuid", dvUuid, "DatasetField", "id", fieldId);

                        // Link Run applies transformation to field
                        graphDao.upsertEdge(GRAPH_NAME, "APPLIES_TRANSFORMATION", "Run", "uuid", runId, "DatasetField", "id", fieldId);
                    }
                }
            }
        }
