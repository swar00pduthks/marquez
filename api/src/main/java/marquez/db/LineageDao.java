  ...
  @SqlQuery(
      ...
      WITH RECURSIVE
        job_io AS (
          SELECT
            j.uuid AS job_uuid,
            COALESCE(j.input_uuids, ARRAY[]::uuid[]) AS inputs,
            COALESCE(j.output_uuids, ARRAY[]::uuid[]) AS outputs
      ...
      LEFT JOIN LATERAL (SELECT j.input_uuids AS inputs, j.output_uuids AS outputs) io ON TRUE
      """)
