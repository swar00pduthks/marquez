          ...
          handle -> {
            ...
            String bulkInsertRunLineage =
                ...
                INSERT INTO run_lineage_denormalized (
                    ...
                    input_dataset_uuid, input_uuids, output_version_uuid, output_dataset_uuid, output_uuids,
                ...
                SELECT
                    ...
                    (SELECT ARRAY_AGG(DISTINCT dataset_uuid) FROM dataset_versions INNER JOIN runs_input_mapping rim ON rim.dataset_version_uuid = dataset_versions.uuid WHERE rim.run_uuid = r.uuid) AS input_uuids,
                    dvout.uuid AS output_version_uuid,
                    dvout.dataset_uuid AS output_dataset_uuid,
                    (SELECT ARRAY_AGG(DISTINCT dataset_uuid) FROM dataset_versions WHERE run_uuid = r.uuid) AS output_uuids,
            ...
            String bulkInsertParentLineage =
                ...
                SELECT DISTINCT
                    ...
                    rim.dataset_version_uuid AS input_version_uuid,
                    dvin.dataset_uuid AS input_dataset_uuid,
                    (SELECT ARRAY_AGG(DISTINCT dv.dataset_uuid) FROM runs_input_mapping rim2 JOIN dataset_versions dv ON dv.uuid = rim2.dataset_version_uuid WHERE rim2.run_uuid IN (SELECT uuid FROM runs WHERE parent_run_uuid = COALESCE(r.parent_run_uuid, r.uuid))) AS input_uuids,
                    dvout.uuid AS output_version_uuid,
                    dvout.dataset_uuid AS output_dataset_uuid,
                    (SELECT ARRAY_AGG(DISTINCT dv.dataset_uuid) FROM dataset_versions dv WHERE dv.run_uuid IN (SELECT uuid FROM runs WHERE parent_run_uuid = COALESCE(r.parent_run_uuid, r.uuid))) AS output_uuids,
                    dvin.namespace_name AS input_dataset_namespace,
                    dvin.dataset_name AS input_dataset_name,
                    dvin.version AS input_dataset_version,
                    dvin.uuid AS input_dataset_version_uuid,
  ...
  private void populateRunParentLineageDenormalized(org.jdbi.v3.core.Handle handle, UUID runUuid) {
    ...
    String insertQuery =
        ...
        SELECT DISTINCT
            ...
            rim.dataset_version_uuid AS input_version_uuid,
            dvin.dataset_uuid AS input_dataset_uuid,
            (SELECT ARRAY_AGG(DISTINCT dv.dataset_uuid) FROM runs_input_mapping rim2 JOIN dataset_versions dv ON dv.uuid = rim2.dataset_version_uuid WHERE rim2.run_uuid IN (SELECT uuid FROM runs WHERE parent_run_uuid = :runUuid)) AS input_uuids,
            dvout.uuid AS output_version_uuid,
            dvout.dataset_uuid AS output_dataset_uuid,
            (SELECT ARRAY_AGG(DISTINCT dv.dataset_uuid) FROM dataset_versions dv WHERE dv.run_uuid IN (SELECT uuid FROM runs WHERE parent_run_uuid = :runUuid)) AS output_uuids,
            dvin.namespace_name AS input_dataset_namespace,
            dvin.dataset_name AS input_dataset_name,
            dvin.version AS input_dataset_version,
            dvin.uuid AS input_dataset_version_uuid,
