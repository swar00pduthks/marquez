...
public class OpenLineageService extends DelegatingDaos.DelegatingOpenLineageDao {
  ...
  public OpenLineageService(BaseDao baseDao, RunService runService, Executor executor) {
    ...
    this.jdbi = baseDao.getHandle().getJdbi();
  }
  ...
  public CompletableFuture<Void> createAsync(LineageEvent event) {
    // V3 Dual Write: Automatically mirror incoming events to the new V3 Graph DB asynchronously
    CompletableFuture.runAsync(
        () -> {
          try {
            new marquez.v3.resources.OpenLineageResourceV3(jdbi, new marquez.v3.db.GraphDao())
                .createLineage(event);
          } catch (Exception e) {
            log.warn("Failed to dual-write LineageEvent to V3 Graph Database", e);
          }
        },
        executor);
